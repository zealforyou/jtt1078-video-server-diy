...
public class ByteUtil {
    ...
    public static byte[] appendBCD(String s, int bcdBytes) {
        while (s.length() < bcdBytes * 2) {
            s = "0" + s;
        }
        byte[] bytes = DeviceUtils.writeBCD(s);
        if (bytes.length != bcdBytes) {
            bytes = Arrays.copyOf(bytes, bcdBytes);
        }
        return bytes;
    }
